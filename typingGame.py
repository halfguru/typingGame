...
class Game():
	...
	def run(self):
		...
		score = 0; error = 0; oldLevel = 1; highlightColor = (self.WHITE); plusOneCheck = False; self.cooldown = 300; self.keypressCheck = False
	...
	def wpm(self):
		...
		wpm_counter = self.BASICFONT.render("speed (words/s): " ,1,self.WHITE)
		speed_counter = self.BASICFONT.render("speed (chars/m): ",1,self.WHITE)
		wpm_counter1 = self.BASICFONT.render(str(self.speed) ,1,self.WHITE)
		speed_counter1 = self.BASICFONT.render(str(self.charspeed),1,self.WHITE)

		self.DISPLAYSURF.blit(wpm_counter, (1040,200))
		self.DISPLAYSURF.blit(speed_counter, (1040,300))
		self.DISPLAYSURF.blit(wpm_counter1, (1050,250))
